...
import com.infamous.infamous_legends.ai.behaviours.PiglinRuntMeleeAttack;
...
public class PiglinRuntAi {

	  public static Brain<?> makeBrain(PiglinRunt p_35100_, Brain<PiglinRunt> p_35101_) {
	   ...
	   public static void initMemories(PiglinRunt p_35095_) {
	   ...
	   private static void initCoreActivity(PiglinRunt p_35112_, Brain<PiglinRunt> p_35113_) {
	   ...
	   private static void initIdleActivity(PiglinRunt p_35120_, Brain<PiglinRunt> p_35121_) {
	      p_35121_.addActivity(Activity.IDLE, 10, ImmutableList.of(new StartAttacking<>(PiglinRuntAi::findNearestValidAttackTarget), createIdleLookBehaviors(), createIdleMovementBehaviors(), new SetLookAndInteract(EntityType.PLAYER, 4)));
	   }

	   private static void initFightActivity(PiglinRunt p_35125_, Brain<PiglinRunt> p_35126_) {
	      ...
	      }), new SetWalkTargetFromAttackTargetIfTargetOutOfReach(1.0F), new PiglinRuntMeleeAttack(20)), MemoryModuleType.ATTACK_TARGET);
	   }

	   private static RunOne<PiglinRunt> createIdleLookBehaviors() {
	      return new RunOne<>(ImmutableList.of(Pair.of(new SetEntityLookTarget(EntityType.PLAYER, 8.0F), 1), Pair.of(new SetEntityLookTarget(EntityType.PIGLIN, 8.0F), 1), Pair.of(new SetEntityLookTarget(EntityTypeInit.PIGLIN_RUNT.get(), 8.0F), 1), Pair.of(new SetEntityLookTarget(8.0F), 1), Pair.of(new DoNothing(30, 60), 1)));
	   }

	   private static RunOne<PiglinRunt> createIdleMovementBehaviors() {
	      return new RunOne<>(ImmutableList.of(Pair.of(new RandomStroll(0.4F), 2), Pair.of(InteractWith.of(EntityType.PIGLIN, 8, MemoryModuleType.INTERACTION_TARGET, 0.6F, 2), 2), Pair.of(InteractWith.of(EntityTypeInit.PIGLIN_RUNT.get(), 8, MemoryModuleType.INTERACTION_TARGET, 0.6F, 2), 2), Pair.of(new StrollToPoi(MemoryModuleType.HOME, 0.6F, 2, 100), 2), Pair.of(new StrollAroundPoi(MemoryModuleType.HOME, 0.6F, 5), 2), Pair.of(new DoNothing(30, 60), 1)));
	   }

	   public static void updateActivity(PiglinRunt p_35110_) {
	      Brain<PiglinRunt> brain = p_35110_.getBrain();
	   ...
	   private static Optional<? extends LivingEntity> getTargetIfWithinRange(AbstractPiglin p_35092_, MemoryModuleType<? extends LivingEntity> p_35093_) {
	      return p_35092_.getBrain().getMemory(p_35093_).filter((p_35108_) -> {
	         return p_35108_.closerThan(p_35092_, 16.0D);
	      });
	   }

	   public static void wasHurtBy(PiglinRunt p_35097_, LivingEntity p_35098_) {
	      if (!(p_35098_ instanceof AbstractPiglin)) {
	         PiglinAi.maybeRetaliate(p_35097_, p_35098_);
	      }
	   }

	   protected static void setAngerTarget(PiglinRunt p_149989_, LivingEntity p_149990_) {
	   ...
	   public static void maybePlayActivitySound(PiglinRunt p_35115_) {
	   ...
	   private static void playActivitySound(PiglinRunt p_35123_) {
